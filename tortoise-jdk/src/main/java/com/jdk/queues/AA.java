...
public class ArrayBlockingQueue<E> extends AbstractQueue<E>
    ...
    private void enqueue(E x) {
        ...
        // 数组跟链表相比的话的确是查找比较方便 插入和删除没有链表灵活
        // 链表的话查找比较麻烦 但是插入和删除比较灵活
        // 根本原因还是数组在内存中的地址是连续的 可以根据下标进行取值
        // 而链表的内存地址并不连续 对于元素的查找需要从头遍历
        items[putIndex] = x;
        // 这个地方加1 下一次直接使用
     ...
     * Call only when holding lock.
     */
    // 具体的出队方法 也就是我们常说的数组实现对列的方式
    // 将当前takeIndex+1 下次取值就是下一个值
    // 那么peek是如何实现的呢？
    private E dequeue() {
        // assert lock.getHoldCount() == 1;
